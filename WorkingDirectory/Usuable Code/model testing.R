...
elo.calculation(getRating(NHL2018.csv[2,10]),(getRating(NHL2018.csv[2,11])),32)
...
for (i in 1:length(nhl.seasons)){
  for (j in 1:nrow(get(nhl.seasons[i]))){
...
elo.test(getRating('Pittsburgh Penguins'), getRating('Washington Capitals'), 32)

hist(teams.table$teams.rating)

summary(teams.table$teams.rating)
...
testing.get <- getRating(NHL2018.csv[1,2])
...
# ratings.update.func2 prints the names and values after each rating
ratings.update.func2 <- function(Winner, Loser){
  Winner.name <<- Winner
  Winner.name <<- trimws(capture.output(Winner.name))
  Loser.name <<- Loser
  Loser.name <<- trimws(capture.output(Loser.name))
